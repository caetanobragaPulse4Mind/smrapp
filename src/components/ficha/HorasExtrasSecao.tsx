...
const SUB_TABS = [
  { id: "secao_1", label: "HE I" },
  { id: "secao_2", label: "HE II" },
  { id: "secao_3", label: "HE III" },
] as const;

type SubTabId = typeof SUB_TABS[number]["id"];

export function HorasExtrasSecao({
  onPrev, onNext, prevLabel, nextLabel,
}: {
  onPrev?: () => void; onNext?: () => void; prevLabel?: string; nextLabel?: string;
}) {
  const { verbas, setVerba, setBase, base } = useProcesso();
  const [sub, setSub] = useState<SubTabId>("secao_1");

  const data = verbas.horas_extras ?? {};
  const secao: SecaoHE = useMemo(() => ({ ...SECAO_DEFAULT, ...(data[sub] ?? {}) }), [data, sub]);

  // Sync jornada_arbitrada / jornada_mista na base conforme qualquer seção
  useEffect(() => {
    const all = ["secao_1", "secao_2", "secao_3"].map((s) => data[s]?.base_apuracao);
    const hasArb = all.includes("ARBITRADO");
    const hasMista = all.includes("MISTO");
    if (base.jornada_arbitrada !== hasArb || base.jornada_mista !== hasMista) {
      setBase((b) => ({ ...b, jornada_arbitrada: hasArb, jornada_mista: hasMista }));
    }
  }, [data, base.jornada_arbitrada, base.jornada_mista, setBase]);

  function update<K extends keyof SecaoHE>(key: K, value: SecaoHE[K]) {
    setVerba("horas_extras", (v) => ({ ...v, [sub]: { ...SECAO_DEFAULT, ...(v?.[sub] ?? {}), [key]: value } }));
  }
  function toggleSumula(sid: string) {
    setVerba("horas_extras", (v) => {
