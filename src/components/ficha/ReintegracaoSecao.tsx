...
const DEFAULT_DATA: ReintegracaoData = {
  base: "",
  percentual: "",
  reflexos: "",
  de: "",
...
export function ReintegracaoSecao({
  onPrev,
  onNext,
  prevLabel,
  nextLabel,
  ...
  const data: ReintegracaoData = useMemo(
    () => ({ ...DEFAULT_DATA, ...(verbas.reintegracao ?? {}) }),
    [verbas.reintegracao],
  );

  function update<K extends keyof ReintegracaoData>(key: K, value: ReintegracaoData[K]) {
    setVerba("reintegracao", (v) => ({ ...DEFAULT_DATA, ...v, [key]: value }));
  }
